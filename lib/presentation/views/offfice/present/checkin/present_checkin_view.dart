...
class PresentCheckInView extends StatefulWidget {
  const PresentCheckInView({super.key});

  @override
  State<PresentCheckInView> createState() => _PresentCheckInViewState();
}

class _PresentCheckInViewState extends State<PresentCheckInView>
        ...
        children: [
          BlocBuilder<PresentDetailCubit, PresentDetailState>(
            builder: (context, state) {
              final image = state.data.fotoKaryawan;
          ...
          BlocBuilder<PresentDetailCubit, PresentDetailState>(
            builder: (context, state) {
              String value = '';
              final checkIn = state.data.waktuCheckIn;
              if (checkIn != null) {
                value = kTimeFullFormat.format(checkIn);
              }

              return HoraTextFieldViewer(
                label: tr('detail_present_clock_check_in'),
                value: value,
          ...
          BlocBuilder<PresentDetailCubit, PresentDetailState>(
            builder: (context, state) {
              String value = state.data.alamatLoc ?? '';

              return HoraTextFieldViewer(
                label: tr('detail_present_location_check_in'),
                value: value,
